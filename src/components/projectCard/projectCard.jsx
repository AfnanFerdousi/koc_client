...
const ProjectCard = ({ data, index, length, myJob }) => {
  const router = useRouter();
  const handleClick = () => {
    router.push(`/job/${data?._id}`);
  };
  const dispatch = useDispatch();
  const posted = "recently";
  const proposals = "0";
  const [showDeleteExperienceModal, setShowDeleteExperienceModal] =
    React.useState(null);
  const editJobLoading = useSelector((state) => state.jobs?.editLoading);
  ...
  const handleSaveJob = () => {
    if (isSaveEnabled) {
      ...
      dispatch(
        editJob({ dynamicParams: { jobId: data?._id }, bodyData: formData })
      )
        .then(() => {
          // After adding experience, fetch the updated profile data
          return dispatch(getJobs(""));
